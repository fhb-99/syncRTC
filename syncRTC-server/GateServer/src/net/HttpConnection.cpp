...
void HttpConnection::start()
{
    ...
    http::async_read(_socket, _buffer, _request, [self](beast::error_code error,
        std::size_t bytes_transferred){
            try
            {
                if(error) 
                {
                    std::cout << "http read error is " << error.message() << std::endl;
                    return;
                }
                //处理读到的数据
...
void HttpConnection::HandleRequest()
{
    ...
    {
        PreParseGetParam();
        //通过单例类logic处理请求，判断是否ok，再返回response
